...
{
    _zipFile = zipOpen_2( (const char*)[zipFile UTF8String], (isAppend ? APPEND_STATUS_ADDINZIP : APPEND_STATUS_CREATE) );
	if( !_zipFile ) 
		return NO;
	return YES;
}
...
//	tm_zip filetime;
	...
	{
		ret = zipOpenNewFileInZip_2( _zipFile,
	...
	else
	{
		...
		ret = zipOpenNewFileInZip3_2( _zipFile,
								  (const char*) [newname cStringUsingEncoding:self.stringEncoding],
								  &zipInfo,
								  NULL,0,
								  NULL,0,
	...
	ret = zipWriteInFileInZip_2( _zipFile, (const void*)[data bytes], dataLen);
	if( ret!=Z_OK )
	{
		return NO;
	}
	ret = zipCloseFileInZip_2( _zipFile );
	if( ret!=Z_OK )
		return NO;
	return YES;
}
...
-(BOOL) CloseZipFile2
{
	...
	if( _zipFile==NULL )
		return NO;
	BOOL ret =  zipClose_2( _zipFile,NULL )==Z_OK?YES:NO;
	_zipFile = NULL;
	return ret;
}
